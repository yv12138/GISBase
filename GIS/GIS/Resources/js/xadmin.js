; !function (win) {
    ...
    //    }
    //}
    Xadmin.prototype.del_tab = function (id,callback) {
        if (id) {
            if (callback) {
                callback();
            }
    ...
    Xadmin.prototype.add_lay_tab = function (title, url, id) {
        element.tabAdd('xbs_tab', {
            title: title
            , content: '<iframe tab-id="' + id + '" name="' + id + '" frameborder="0" src="' + url + '" scrolling="yes" class="x-iframe"></iframe>'
            , id: id
        })
    }
	/**
...
layui.use(['layer', 'element', 'jquery'], function () {
    ...
    $(".layui-tab-title").on('contextmenu', 'li', function (event) {
        if ($(this).attr('lay-id') == "home") {
            ...
            $("#tab_right dd[data-type=this]").css('display', 'none');
            $("#tab_right dd[data-type=all]").css('display', 'none');
    ...
    $('#tab_right').on('click', 'dd', function (event) {
        ...
        if (data_type == 'this') {
            ...
            //$('.layui-tab-title li[lay-id!=' + lay_id + ',lay-id!=home]').find('.layui-tab-close').click();
            for (var i = 0; i < $('.layui-tab-title li').length; i++) {
                if ($('.layui-tab-title li').eq(i).attr('lay-id') != lay_id && $('.layui-tab-title li').eq(i).attr('lay-id') != "home") {
                    $('.layui-tab-title li').eq(i).find('.layui-tab-close').click()
                }         
            }
        } else if (data_type == 'all') {
            $('.layui-tab-title li[lay-id!=home]').find('.layui-tab-close').click();
        }
        $('#tab_right').hide();
        $('#tab_show').hide();
    })
